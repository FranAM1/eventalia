<x-app-layout>
    ...
    <h1 class="text-3xl font-bold mt-8"> Editando el evento {{ $event->name }} </h1>
    @if(session('success'))
    ...
    <form action="{{ route('event.update', $event) }}" method="POST"
        ...
        <div class="flex flex-col items-start font-bold w-full">
            ...
            <textarea name="description" id="description" cols="30" rows="2"
                class="text-black rounded-sm w-full border-2">{{ old('description', $event->description) }}</textarea>
            @error('description')
            <p class="text-red-500">{{ $message }}</p>
            @enderror
        </div>
            ...
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <select name="category" id="category" class="text-black rounded-sm w-full border-2">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category', $event->category_id) == $category->id ? 'selected' : ''}}>{{ $category->category_name }}</option>
                    @endforeach
                </select>
                @error('category')
                    <p class="text-red-500">{{ $message }}</p>
        ...
        <section class="grid grid-cols-2 w-full gap-4">
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <select name="province" id="province" class="text-black rounded-sm w-full border-2">
                    ...
                    @foreach ($provinces as $province)
                        <option value="{{ $province->id }}" {{ old('province', $event->province_id) == $province->id ? 'selected' : ''}}>{{ $province->province_name }}</option>
                    @endforeach
                </select>
                @error('province')
                <p class="text-red-500">{{ $message }}</p>
            ...
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <select name="city" id="city" class="text-black rounded-sm w-full border-2">
                    ...
                    @if(old('province', $event->province_id))
                        @foreach($provinces->find(old('province', $event->province_id))->cities as $city)
                            <option class="text-black"
                            value="{{ $city->id }}" {{ old('city', $event->city_id) == $city->id ? 'selected' : '' }}>
                                {{ $city->city_name }}
                            </option>
                        @endforeach
                    @endif
        ...
        <section class="grid grid-cols-2 w-full gap-4">
            ...
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <input type="datetime-local" name="end_date" id="end_date" cols="30" rows="2"
                    value="{{ old('end_date', $event->end_date) }}" class="text-black rounded-sm w-full border-2">
                @error('end_date')
                <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
