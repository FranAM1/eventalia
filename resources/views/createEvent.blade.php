<x-app-layout>
    ...
    @if(session('success'))
        <div class="text-green-500 font-bold mt-8">
            {{ session('success') }}
        </div>
    @endif
            ...
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <select name="category" id="category" class="text-black rounded-sm w-full border-2">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ old('province') == $category->id ? 'selected' : ''}}>{{ $category->category_name }}</option>
                    @endforeach
                </select>
                @error('category')
                    <p class="text-red-500">{{ $message }}</p>
        ...
        <section class="grid grid-cols-2 w-full gap-4">
            ...
            <div class="flex flex-col items-start font-bold w-full">
                ...
                <select name="city" id="city" class="text-black rounded-sm w-full border-2">
                    ...
                    @if(old('province'))
                        @foreach($provinces->find(old('province'))->cities as $city)
